...
**For Engineer (Team):** Use the `Task` tool to dispatch the lead Engineer as a subagent. The lead Engineer will FIRST dispatch a **Senior Engineer** sub-agent (via the `Agent` tool) to read the full architecture and produce a granular task list with a coverage map (every component, endpoint, and entity in `04-architecture.md` accounted for, every task sized small enough that a sub-agent can implement it in one shot, with the relevant architecture excerpt embedded verbatim in each task entry). The lead Engineer then dispatches engineer sub-agents in parallel waves — **one sub-agent per task**, not per domain — using the `Agent` tool. Each sub-agent receives its task's architecture excerpt directly so it never needs to read the architecture doc itself. After all waves complete, the lead Engineer runs an **Architecture Coverage Check** (walks `04-architecture.md` section by section and verifies every named component, endpoint, entity, and file actually exists in code) before reporting done. Invoke the Writer to persist the implementation plan.
